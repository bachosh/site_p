...
CSV.foreach( csv_file_pth , :headers => true, :quote_char => "|") do |row|
begin	
puts "--------------------------NEW CSV ROW--------------------------"
...
puts col
if !col[1].nil?
rowhash[col[0].gsub(/"/, '')]=col[1].gsub(/"/, '') 
else
rowhash[col[0].gsub(/"/, '')]=col[1]
end 


puts col[0].gsub(/"/, '')

fl_column=Filterlink.find_by(csv_header_name: col[0].to_s.gsub(/"/, ''))
fl_column = fl_column.column_name if !fl_column.nil?

  if !fl_column.nil?
  fil_column_val=filtercopart.send fl_column 

  if !fil_column_val.nil? && !fil_column_val.empty? && fil_column_val == col[1].gsub(/"/, '')
  	then isgood = true
  elsif !fil_column_val.nil? && !fil_column_val.empty? && fil_column_val != col[1].gsub(/"/, '') 
    then 
      isbad = true
  end   	 
